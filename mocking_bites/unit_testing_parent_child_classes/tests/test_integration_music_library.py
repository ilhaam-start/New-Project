...
#Add one track tests if the keyword is in the Music Library, when search is called return the match.
def test_keyword_in_music_library_return_true():
    musiclibrary = MusicLibrary()
    track_1 = Track("song 1", "singer 1")
    musiclibrary.add(track_1)
    assert musiclibrary.search("singer 1") == [track_1]


#Add two tracks tests if the word keyword is in the Music Library, return match
def test_add_two_tracks_keyword_in_music_library_return_true():
    musiclibrary = MusicLibrary()
    track_1 = Track("song 1", "singer 1")
